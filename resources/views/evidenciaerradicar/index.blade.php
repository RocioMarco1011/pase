...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">
            EVIDENCIAS DE LA ACCIÓN DE ERRADICAR
        </h2>
        @if(isset($estrategia))
        ...
        @endif
        @if(isset($accionErradicar))
            <p class="font-semibold text-sm text-gray-800 leading-tight text-center">
                Acción de Erradicar: {{ $accionErradicar->accion }}
            </p>
        @endif
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">                  
                    <div class="mb-4">
                        <a href="{{ route('estrategiaserradicar.accionerradicar.index', ['estrategia' => $estrategia->id]) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                            Regresar a  las Acciones de Erradicar
                        </a>
                    </div>
                    
                    <div class="mb-4">
                        <a href="{{ route('evidenciaerradicar.create', ['estrategiaId' => $estrategia->id, 'accionErradicarId' => $accionErradicar->id]) }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                            Añadir Evidencia
                        </a>
                    </div>
                    <table class="min-w-full border-collapse border-2 border-gray-300">
                        <thead>
                            <tr>
                                ...
                                @can('EliminarEvidenciaErradicar')
                                <th class="py-2 px-4 text-lg border-b border-r"></th>
                                @endcan
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($evidencias as $evidencia)
                                <tr>
                                    ...
                                    <td class="py-2 px-4 text-lg border-b border-r">
                                        <a href="{{ route('evidenciaerradicar.edit', ['estrategiaId' => $estrategia->id, 'accionErradicarId' => $accionErradicar->id, 'evidenciaId' => $evidencia->id]) }}" class="text-indigo-600 hover:text-indigo-900">
                                            Editar
                                        </a>
                                    </td>
                                    @can('EliminarEvidenciaErradicar')
                                    <td class="py-2 px-4 text-lg border-b border-r">
                                        <form action="{{ route('evidenciaerradicar.destroy', ['estrategiaId' => $estrategia->id, 'accionErradicarId' => $accionErradicar->id, 'evidenciaId' => $evidencia->id]) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
