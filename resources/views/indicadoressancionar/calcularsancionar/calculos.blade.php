            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex mb-4">
                            <a href="{{ route('indicadoressancionar.calcularsancionar.calcular', ['indicadorsancionar' => $indicadorsancionar->id]) }}"
                            ...
                            @can('VerFormulaSancionar')
                            @if($calculo->count() > 0)
                                <a href="{{ route('indicadoressancionar.calcularsancionar.show', ['id' => $calculo[0]->id]) }}" class="inline-flex items-center justify-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150">
                                    Ver Fórmula
                                </a>
                            @endif
                            @endcan
                            @if($calculo->count() > 0)
                                <a href="{{ route('descargar.pdf.sancionar', ['indicadorsancionar' => $indicadorsancionar->id]) }}" class="inline-flex items-center justify-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray disabled:opacity-25 transition ease-in-out duration-150" style="margin-left: 10px;">
                                    Descargar PDF
                                </a>
                            @endif
                        </div>
                            ...
                            @foreach($calculo->reverse() as $calculo)
                                <div class="card">
                                    <div class="card-body" style="background-color: {{ $indicadorsancionar->semaforo === 'Verde > 0 - Amarillo = 0 - Rojo < 0' ? ($calculo->resultado > 0 ? 'green' : ($calculo->resultado == 0 ? 'yellow' : 'red')) : ($calculo->resultado < 0 ? 'green' : ($calculo->resultado == 0 ? 'yellow' : 'red')) }}">
                                        ...
                                        @can('EliminarCalculoSancionar')
                                        <!-- Delete Formula Form -->
                                        <form method="POST" action="{{ route('indicadoressancionar.calcularsancionar.destroy', ['calculo' => $calculo->id]) }}">
                                            @csrf
                                            @method('DELETE')
