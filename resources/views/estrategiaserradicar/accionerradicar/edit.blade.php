    ...
    <x-app-layout>
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight text-center">
                EDITAR ACCIÓN DE ERRADICACIÓN EN ESTRATEGIA
            </h2>
        </x-slot>
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <form method="POST" action="{{ route('estrategiaserradicar.accionerradicar.update', ['estrategiaId' => $estrategia->id, 'accionErradicarId' => $accionErradicar->id]) }}">
                            @csrf
                            @method('PUT') 
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" name="accion" id="accion" class="mt-1 p-2 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm" value="{{ $accionErradicar->accion }}">
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <select name="tipo" id="tipo" class="mt-1 p-2 block w-full border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm">
                                    <option value="General" {{ $accionErradicar->tipo === 'General' ? 'selected' : '' }}>General</option>
                                    <option value="Especifica" {{ $accionErradicar->tipo === 'Especifica' ? 'selected' : '' }}>Específica</option>
                                </select>
                            </div>
